...
import { isStoryScene, normalizeProject } from './model';
...
export async function importProject(file: File): Promise<ContinuumProject> {
  ...
  return normalizeProject(value as ContinuumProject);
}
...
export function exportStoryboardHtml(project: ContinuumProject): void {
  ...
  const sceneCards = scenes.map((scene) => {
    ...
    const image = scene.images?.[0] ? `<img class="scene-image" src="${scene.images[0].dataUrl}" alt="${escapeHtml(scene.images[0].name)}">` : '';
    const links = (scene.links ?? []).length ? `<section class="references"><b>References</b>${scene.links.map((link) => `<a href="${escapeHtml(link.url)}" target="_blank" rel="noreferrer">${escapeHtml(link.label)}</a>`).join('')}</section>` : '';
    return `
      <article class="scene">
        ${image}
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${escapeHtml(project.title)} — Storyboard</title><style>
    :root{font-family:Inter,ui-sans-serif,system-ui,sans-serif;color:#172126;background:#edf0ea}body{margin:0}main{max-width:1040px;margin:auto;padding:56px 24px 80px}.eyebrow{letter-spacing:.16em;text-transform:uppercase;font-size:12px;color:#60706f}h1{font-family:Georgia,serif;font-size:52px;margin:10px 0 12px}.intro{max-width:760px;font-size:18px;line-height:1.6;color:#4c5a5c}.scene{background:#fff;border:1px solid #d8ddd7;border-radius:20px;padding:26px;margin-top:22px;box-shadow:0 8px 24px #1b2b2710;overflow:hidden}.scene-image{width:calc(100% + 52px);max-height:360px;object-fit:cover;margin:-26px -26px 24px}.scene header{display:flex;justify-content:space-between;color:#657573;font-size:13px;text-transform:uppercase;letter-spacing:.08em}.scene h2{font-family:Georgia,serif;font-size:30px;margin:16px 0 8px}.summary{color:#53615f;font-size:16px}dl{display:grid;grid-template-columns:160px 1fr;gap:10px 20px;border-top:1px solid #e4e8e3;padding-top:20px}dt{font-weight:700;color:#5d6d69}dd{margin:0;line-height:1.5}aside{background:#f4efe4;border-radius:12px;padding:16px 18px;margin-top:20px}aside p{margin:6px 0 0;line-height:1.5}.references{display:flex;gap:10px;align-items:center;flex-wrap:wrap;border-top:1px solid #e4e8e3;margin-top:20px;padding-top:16px}.references a{color:#295f67;text-decoration:none;border:1px solid #cfd9d6;border-radius:999px;padding:7px 10px;font-size:13px}@media(max-width:640px){h1{font-size:38px}dl{grid-template-columns:1fr;gap:3px}dd{margin-bottom:10px}}
