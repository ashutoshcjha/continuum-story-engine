import type { ContinuumProject, StoryEntity, StoryRelationship, StoryScene } from './model';
...
import { normalizeScifiProject, sceneContinuityWarnings } from './scifi';
...
const normalizeContinuum = (project: ContinuumProject) => normalizeScifiProject(normalizeStoryLogic(normalizeProject(project)));

export function exportProject(project: ContinuumProject): void {
  const normalized = normalizeContinuum(project);
...
export async function importProject(file: File): Promise<ContinuumProject> {
  ...
  return normalizeContinuum(value as ContinuumProject);
}
...
const readable = (value?: string) => value ? value.replaceAll('-', ' ') : '—';
...
function continuityHtml(project: ContinuumProject, scene: StoryScene): string {
  ...
  const environmentRows = environment ? [
    ['Setting', readable(environment.setting)],
    ['Atmosphere', readable(environment.atmosphere)],
    ['Gravity', readable(environment.gravity)],
    ['Radiation', readable(environment.radiation)],
    ['Temperature', environment.temperature || '—'],
    ['Suit required', environment.suitRequired ? 'Yes' : 'No'],
    ['Airlock required', environment.airlockRequired ? 'Yes' : 'No'],
    ['Communications', readable(environment.communications)],
    ['Visibility', readable(environment.visibility)],
    ['Local time', environment.localTime || '—'],
    ['Weather', environment.weather || '—'],
  ] : [];
  const travelRows = travel && (travel.mode || travel.originId || travel.destinationId || travel.route) ? [
    ['Travel', `${nameOf(project.entities, travel.originId)} → ${nameOf(project.entities, travel.destinationId ?? scene.scene.locationId)}`],
    ['Mode', travel.mode || '—'],
    ['Route', travel.route || '—'],
    ['Duration', travel.durationMinutes ? `${travel.durationMinutes} minutes` : '—'],
    ['Authorization', travel.authorization || '—'],
    ['Exposure', travel.environmentalExposure || '—'],
    ['Complications', travel.complications || '—'],
  ] : [];
  if (!environmentRows.length && !travelRows.length && !warnings.length) return '';
  const rows = [...environmentRows, ...travelRows]
    .filter(([, value]) => value !== 'unknown')
    .map(([key, value]) => `<dt>${escapeHtml(key)}</dt><dd>${escapeHtml(value)}</dd>`)
    .join('');
  const warningHtml = warnings.length
    ? `<div class="continuity-warnings"><b>Continuity checks</b><ul>${warnings.map((warning) => `<li class="${warning.severity}">${escapeHtml(warning.message)}</li>`).join('')}</ul></div>`
    : '';
  return `<section class="continuity"><header><span>Science-fiction continuity</span><h4>Environment and travel</h4></header>${rows ? `<dl>${rows}</dl>` : ''}${environment?.notes ? `<p>${escapeHtml(environment.notes)}</p>` : ''}${warningHtml}</section>`;
}
...
export function exportStoryboardHtml(project: ContinuumProject): void {
  ...
  const chapterSections = chapters.map((chapter) => {
    ...
    const sceneCards = scenes.map((scene) => {
      ...
      const continuity = continuityHtml(normalized, scene);
